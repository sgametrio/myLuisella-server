...
//uncomment this if you don't handle ORIGIN in configuration file
    ...
    try
    {
        ...
        $statement = $db -> prepare("SELECT orderId FROM `luisella`.`order` WHERE tableId = :tableId AND :tableId IN (SELECT DISTINCT tableId FROM `food&order`, `order` WHERE (status = 1 OR status = 2) AND `food&order`.orderId = order.orderId ORDER BY orderedTime DESC)");
